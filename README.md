...
Any decent level of accuracy from this set of formula can only be achieved through a consistent estimate of the 3 bulk transfer coefficients: $C_D$, $C_E$, and $C_H$ (drag, evaporation, and sensible heat coefficients).
In **AeroBulk**, these bulk coefficients can be estimated thanks to a collection of _bulk parameterizations_ a.k.a _bulk algorithms_, which relate the value of these coefficients to the near-surface atmospheric stability, the wind speed, and (ideally) the roughness of the sea surface.
