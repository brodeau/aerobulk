...
L_ALGOS = [ 'nemo'     ,   'an05'   ]
l_color = [  '#ffed00' , '#008ab8'  ] ; # colors to differentiate algos on the plot
l_width = [     3      ,    2       ] ; # line-width to differentiate algos on the plot
l_style = [    '-'     ,   '-'      ] ; # line-style
...
L_VNEM  = [     'Qsen'     ,  'Tau'      ]
L_VARO  = [     'Qsen'     ,  'Tau'      ] ; # name of variable on figure
L_VARL  = [  r'$Q_{sens}$' , r'$|\tau|$' ] ; # name of variable in latex mode
L_VUNT  = [  r'$W/m^2$'    , r'$N/m^2$'  ]
L_VMAX  = [      75.       ,    1.2      ]
L_VMIN  = [    -125.       ,    0.       ]
L_ANOM  = [     True       ,   True      ]
...
for jv in range(nb_var-1):
    ...
    for ja in range(nb_algos-1):
        ...
        if nmp.sum(xFa[:,ja]) == 0.0:
            ...
            #plt.legend(bbox_to_anchor=(0.45, 0.2), ncol=1, shadow=True, fancybox=True)
            ax1.annotate(cvar_lnm, xy=(0.3, 0.97), xycoords='axes fraction',  bbox={'facecolor':'w', 'alpha':1., 'pad':10}, zorder=50, **font_inf)
            plt.savefig('diff_skin-noskin_'+L_VARO[jv]+'_'+calgo+'.'+fig_ext, dpi=int(rDPI), transparent=False)
            plt.close(jv)
