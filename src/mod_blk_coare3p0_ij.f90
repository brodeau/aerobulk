...
MODULE mod_blk_coare3p0
   ...
   PUBLIC :: COARE3P0_INIT, TURB_COARE3P0, charn_coare3p0, psi_m_coare, psi_h_coare
...
CONTAINS
   ...
   SUBROUTINE coare3p0_init(nx, ny, l_use_cs, l_use_wl)
      !!---------------------------------------------------------------------
   ...
   SUBROUTINE turb_coare3p0( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl, &
      ...
      IF( kt == nit000 ) CALL COARE3P0_INIT( Ni, Nj, l_use_cs, l_use_wl )
      ...
      !PRINT *, 'LOLO: mod_blk_coare3p0.f90 before iter loop: z0 =', REAL(zz0,4)
      !STOP
   ...
   FUNCTION charn_coare3p0_sclr( pwnd )
      ...
      !!-------------------------------------------------------------------
      charn_coare3p0_sclr = MAX( MIN( 0.0017_wp*pwnd - 0.005_wp , charn0_max) , 0._wp )
      !!
   ...
   FUNCTION charn_coare3p0_vctr( pwnd )
      ...
      REAL(wp), PARAMETER :: charn0_max = 0.028  !: value above which the Charnock parameter levels off for winds > 18 m/s
      !!-------------------------------------------------------------------
      charn_coare3p0_vctr = MAX( MIN( 0.0017_wp*pwnd - 0.005_wp , charn0_max) , 0._wp )
   ...
   FUNCTION charn_coare3p0_wave( pus, pwsh, pwps )
      !!-------------------------------------------------------------------
      ...
      REAL(wp), INTENT(in) :: pwsh  ! significant wave height       [m]
      REAL(wp), INTENT(in) :: pwps  ! phase speed of dominant waves [m/s]
      REAL(wp)             :: charn_coare3p0_wave
      !!-------------------------------------------------------------------
      charn_coare3p0_wave = ( pwsh*0.2_wp*(pus/pwps)**2.2_wp ) * grav/(pus*pus)
      !!
   END FUNCTION charn_coare3p0_wave
