...
MODULE mod_blk_ecmwfn
   ...
   !!
   !!       Routine turb_ecmwfn maintained and developed in AeroBulk
   ...
   USE mod_skin_new    !: cool-skin parameterization

   IMPLICIT NONE
   PRIVATE

   PUBLIC :: ECMWFN_INIT, TURB_ECMWFN
...
CONTAINS
   ...
   SUBROUTINE ecmwfn_init(l_use_cs, l_use_wl)
      !!---------------------------------------------------------------------
      !!                  ***  FUNCTION ecmwfn_init  ***
      !!
      !! INPUT :
      !! -------
      ...
      IF ( l_use_cs ) THEN
         ierr = 0
         PRINT *, ' *** ecmwfn_init: CS => allocating dT_cs :', jpi,jpj
         ALLOCATE ( dT_cs(jpi,jpj), STAT=ierr )
         !IF( ierr > 0 ) STOP ' ECMWFN_INIT => allocation of dT_cs and Qnt_ac failed!'
         dT_cs(:,:) = -0.25_wp  ! First guess of skin correction
         PRINT *, ' *** dT_cs allocated!'
      END IF
   END SUBROUTINE ecmwfn_init



   SUBROUTINE turb_ecmwfn( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl,  &
      ...
      CHARACTER(len=40), PARAMETER :: crtnm = 'turb_ecmwfn@mod_blk_ecmwfn.f90'
      !!----------------------------------------------------------------------------------
      ...
      DO j_itt = 1, nb_itt
         ...
         IF( l_use_cs ) THEN
            ...
            CALL CS_NEW( Qsw, ztmp1, u_star, zsst )  ! Qnsol -> ztmp1
         ...
         IF( l_use_wl ) THEN
            ...
            CALL WL_NEW( Qsw, ztmp1, u_star, zsst )
