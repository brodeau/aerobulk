...
CONTAINS
   ...
   SUBROUTINE turb_ecmwf_ij( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl,    &
      ...
      ztmp0  = vkarmn/(LOG(zu/z0t) - psi_h_ecmwf_ij(func_h))

      u_star = MAX ( Ubzu*vkarmn/(LOG(zu) - LOG(z0)  - psi_m_ecmwf_ij(func_h)) , 1.E-9 )  !  (MAX => prevents FPE from stupid values from masked region later on)
      t_star = dt_zu*ztmp0
      q_star = dq_zu*ztmp0
      ...
      IF( .NOT. l_zt_equal_zu ) THEN
         ...
         ztmp0 = psi_h_ecmwf_ij(func_h) - psi_h_ecmwf_ij(zt*func_h/zu)    ! zt*func_h/zu == zeta_t
      ...
      ztmp0 = zu*Linv
      func_m = LOG(zu) - LOG(z0)  - psi_m_ecmwf_ij(ztmp0) + psi_m_ecmwf_ij( z0*Linv)
      func_h = LOG(zu) - LOG(z0t) - psi_h_ecmwf_ij(ztmp0) + psi_h_ecmwf_ij(z0t*Linv)
      ...
      DO jit = 1, nb_iter
         ...
         func_m = LOG(zu) -LOG(z0) - psi_m_ecmwf_ij(zu*Linv) + psi_m_ecmwf_ij(z0*Linv) ! LB: should be "zu+z0" rather than "zu" alone, but z0 is tiny wrt zu!
         ...
         IF( .NOT. l_zt_equal_zu ) THEN
            ...
            func_h = psi_h_ecmwf_ij(zu*Linv) ! temporary array !!!
            func_m = psi_h_ecmwf_ij(zt*Linv) ! temporary array !!!

            ztmp2  = psi_h_ecmwf_ij(z0t*Linv)
            ztmp0  = func_h - ztmp2
      ...
      ztmp2 = LOG(zu/z0q) - psi_h_ecmwf_ij(zu*Linv) + psi_h_ecmwf_ij(z0q*Linv)   ! func_q
