...
MODULE mod_blk_ncar_ij
   ...
   !!
   !!       Routine turb_ncar_ij maintained and developed in AeroBulk
   ...
   PUBLIC :: TURB_NCAR_IJ, cd_n10_ncar_ij, ch_n10_ncar_ij, ce_n10_ncar_ij, psi_m_ncar_ij, psi_h_ncar_ij

   !!----------------------------------------------------------------------
CONTAINS

   SUBROUTINE turb_ncar_ij( zt, zu, sst, t_zt, ssq, q_zt, U_zu,   &
      &                     Cd, Ch, Ce, t_zu, q_zu, Ubzu,         &
      &                    CdN, ChN, CeN, xz0, xu_star, xL, xUN10 )
      !!----------------------------------------------------------------------------------
      !!                      ***  ROUTINE  turb_ncar_ij  ***
      !!
      ...
      CHARACTER(len=40), PARAMETER :: crtnm = 'turb_ncar_ij@mod_blk_ncar_ij.f90'
      !!----------------------------------------------------------------------------------
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            zCdN = cd_n10_ncar_ij( Ubzu(ji,jj) )
            zsqrt_CdN = SQRT( zCdN )

            !! Initializing transf. coeff. with their first guess neutral equivalents :
            Cd(ji,jj) = zCdN
            Ce(ji,jj) = ce_n10_ncar_ij( zsqrt_CdN )
            Ch(ji,jj) = ch_n10_ncar_ij( zsqrt_CdN , zstab )   ! zstab is stability (1/0)
            zsqrt_Cd = zsqrt_CdN
            ...
            DO jit = 1, nb_iter
               ...
               IF( .NOT. l_zt_equal_zu ) THEN
                  ...
                  ztmp = zlog1 + psi_h_ncar_ij(zeta_u) - psi_h_ncar_ij(zeta_t)
               ...
               ztmp = ( zlog2 - psi_h_ncar_ij(zeta_u) ) / vkarmn / zsqrt_CdN
   ...
   END SUBROUTINE turb_ncar_ij


   FUNCTION cd_n10_ncar_ij( pw10 )
      !!----------------------------------------------------------------------------------
   ...
   FUNCTION ch_n10_ncar_ij( psqrtcdn10 , pstab )
      ...
      IF( (pstab < -0.00001).OR.(pstab >  1.00001) ) THEN
         PRINT *, 'ERROR: ch_n10_ncar_ij@mod_blk_ncar_ij.f90: pstab ='
         PRINT *, pstab
         STOP
      END IF
      !
      ch_n10_ncar_ij = MAX( 1.e-3_wp * psqrtcdn10*( 18._wp*pstab + 32.7_wp*(1._wp - pstab) )  , Cx_min )   ! Eq. (9) & (12) Large & Yeager, 2008
      !
   END FUNCTION ch_n10_ncar_ij

   FUNCTION ce_n10_ncar_ij( psqrtcdn10 )
      !!----------------------------------------------------------------------------------
      !! Estimate of the neutral heat transfer coefficient at 10m      !!
      !! Origin: Large & Yeager 2008, Eq. (9) and (13)
      !!----------------------------------------------------------------------------------
      REAL(wp)             :: ce_n10_ncar_ij
      REAL(wp), INTENT(in) :: psqrtcdn10 ! sqrt( CdN10 )
      !!----------------------------------------------------------------------------------
      ce_n10_ncar_ij = MAX( 1.e-3_wp * ( 34.6_wp * psqrtcdn10 ) , Cx_min )
      !
   END FUNCTION ce_n10_ncar_ij


   FUNCTION psi_m_ncar_ij( pzeta )
      !!----------------------------------------------------------------------------------
      ...
      !
      psi_m_ncar_ij =          zstab  * zpsi_stab &  ! (zta > 0) Stable
         &              + (1._wp - zstab) * zpsi_unst    ! (zta < 0) Unstable
      !
   END FUNCTION psi_m_ncar_ij


   FUNCTION psi_h_ncar_ij( pzeta )
      !!----------------------------------------------------------------------------------
      ...
      zstab = 0.5_wp + SIGN(0.5_wp, zta) ! zta > 0 => zstab = 1
      !
      psi_h_ncar_ij =          zstab  * zpsi_stab &  ! (zta > 0) Stable
         &              + (1._wp - zstab) * zpsi_unst    ! (zta < 0) Unstable
      !
   END FUNCTION psi_h_ncar_ij

   !!======================================================================
END MODULE mod_blk_ncar_ij
