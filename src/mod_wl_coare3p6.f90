...
MODULE mod_wl_coare3p6
   ...
   !*******************  set constants  ****************
   REAL(8), PARAMETER :: tdk = 273.16   !Converts to Kelvin
   REAL(8), PARAMETER :: Rgas = 287.1   !Universal gas constant
   REAL(8), PARAMETER :: cpa = 1004.67  !Specific heat of air at constant pressure
   REAL(8), PARAMETER :: cpw = 4000     !Specific heat of water
   REAL(8), PARAMETER :: rhow = 1022    !Density of water
   REAL(8), PARAMETER :: visw = 1e-6    !Viscosity of water
   REAL(8), PARAMETER :: be = 0.026
   REAL(8), PARAMETER :: tcw = 0.6
   REAL(8), PARAMETER :: grav = 9.8
   REAL(8), PARAMETER :: rich = .65_wp       !critical Richardson number

   REAL(8), PARAMETER :: z_sst = 1._wp   !: depth at which bulk SST is taken...
...
CONTAINS
   ...
   SUBROUTINE WL_COARE3P6( pSST, pTau, pQsw, pQnsol, pdT, q_zu, pTau_ac, pQ_ac, itime_b, itime_n )

      REAL(wp), INTENT(in)    :: pSST     ! bulk SST at depth z_sst [K]
      REAL(wp), INTENT(in)    :: pTau     ! wind stress [N/m^2]
      !....

      REAL(wp), INTENT(inout) :: pdT      ! dT due to warming at depth of pSST such that pSST_true = pSST + pdT
      REAL(wp), INTENT(inout) :: pTau_ac  ! time integral / accumulated momentum Tauxdt => [N.s/m^2] (reset to zero every midnight)
      REAL(wp), INTENT(inout) :: pQ_ac    ! time integral / accumulated heat stored by the warm layer Qxdt => [J/m^2] (reset to zero every midnight)
      ...
      REAL(wp) :: dT_wl, dz_max, dz_wl, pdT, Qabs, zfs
      REAL(wp) :: zqac
      ...
      nx = LEN(yday)         !# of lines of data

      DO ibg = 1, nx         !major read loop

         !*****  variables for warm layer  ***
         cpv = cpa*(1._wp + 0.84_wp*q_zu)
         !visa = 1.326e-5*(1+6.542e-3*t+8.301e-6*t*t-4.84e-9*t*t*t)

         Al = 2.1e-5*(pSST-rt0 + 3.2_wp)**0.79
         ctd1 = SQRT(2._wp*rich*cpw/(Al*grav*rhow))        !mess-o-constants 1
         ctd2 = SQRT(2._wp*Al*grav/(rich*rhow))/(cpw**1.5) !mess-o-constants 2
         ...
         IF ( (itime_n <= 21600).OR.(jump == 0) ) THEN  ! (21600 == 6am)
            ...
            ELSE
               ...
               Qabs = zfs*pQsw - pQnsol       ! tot heat absorbed in warm layer

               IF ( (Qabs >= 50._wp).OR.(jamset == 1) ) THEN         ! Check for threshold
                  jamset = 1                                         ! indicates threshold crossed
                  pTau_ac = pTau_ac + MAX(.002_wp , pTau)*dtime      ! momentum integral

                  IF ( pQ_ac+Qabs*dtime > 0._wp ) THEN                !check threshold for warm layer existence
                     !******************************************
                     ...
                     DO jl = 1, 5                           !loop 5 times for zfs
                        ...
                        qjoule = (zfs*pQsw - pQnsol)*dtime
                        zqac = pQ_ac + qjoule
                        IF (zqac > 0._wp)  dz_wl = MIN( dz_max , ctd1*pTau_ac/SQRT(zqac)) !Compute warm-layer depth
                     END DO
                  ...
                  pQ_ac = zqac !heat integral

                  !*******  compute dt_warm  ******
                  IF (pQ_ac > 0._wp) THEN
                     dT_wl = ctd2*pQ_ac**1.5/pTau_ac
                  ELSE
                     dT_wl = 0
                  END IF
