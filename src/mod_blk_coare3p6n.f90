...
MODULE mod_blk_coare3p6n
   ...
   !!
   !!       Routine turb_coare3p6n maintained and developed in AeroBulk
...
CONTAINS
   ...
   SUBROUTINE coare3p6n_init(l_use_cs, l_use_wl)
      !!---------------------------------------------------------------------
      !!                  ***  FUNCTION coare3p6n_init  ***
      !!
      !! INPUT :
      !! -------
      ...
      IF ( l_use_wl ) THEN
         ierr = 0
         PRINT *, ' *** coare3p6n_init: WL => allocating Tau_ac, Qnt_ac, and Hz_wl :', jpi,jpj
         !ALLOCATE ( Tau_ac(jpi,jpj) , Qnt_ac(jpi,jpj), Hz_wl(jpi,jpj), dT_wl(jpi,jpj), STAT=ierr )
         ALLOCATE ( dT_wl(jpi,jpj), STAT=ierr )
         !IF( ierr > 0 ) STOP ' COARE3P6N_INIT => allocation of Tau_ac and Qnt_ac failed!'
         !Tau_ac(:,:) = 0._wp
         !Qnt_ac(:,:) = 0._wp
         !Hz_wl(:,:)  = Hwl_max
         dT_wl(:,:)  = 0._wp
   ...
   SUBROUTINE turb_coare3p6n( kt, zt, zu, T_s, t_zt, q_s, q_zt, U_zu, l_use_cs, l_use_wl,  &
      ...
      IF ( kt == nit000 ) CALL COARE3P6N_INIT(l_use_cs, l_use_wl) ! allocation of accumulation arrays
      ...
      DO j_itt = 1, nb_itt
         ...
         IF( l_use_wl ) THEN
            ...
            !CALL WL_NEW( Qsw, ztmp1, zeta_u, zsst, plong, isecday_utc, MOD(nb_itt,j_itt) )
